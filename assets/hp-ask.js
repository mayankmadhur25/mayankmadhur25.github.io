/* Healthcare Pulse — floating Ask widget
   Reuses the same knowledge base and Cloudflare Worker as /ask/.
   Loaded sitewide from _layouts/default.html. */
(function(){
  if (window.__hpAskLoaded) return;
  window.__hpAskLoaded = true;
  // don't duplicate the experience on the Ask page itself
  if (location.pathname.indexOf('/ask') === 0) return;

  var WORKER_URL = "https://healthcare-pulse-ask.mayankmadhur25.workers.dev";

var EDITIONS = [
    {ed:63,url:"/edition-63-oncology-margin-burden.html",title:"Hospitals don't follow patients, they follow the rupee",cat:"Strategy",kw:"oncology cancer arpob margin disease burden private hospital capacity build linear accelerator radiation chemotherapy day care max healthcare service mix chronic under-built capex",sum:"Cancer is a sliver of India's disease burden, yet oncology is the number one thing private hospitals are building. The reason is not medicine, it is ARPOB. Three proofs including Max Healthcare's own disclosure, the five-layer economics of a cancer patient, and why the chronic burden stays under-built."},
    ...
    {ed:64,url:"/edition-64-csection-nfhs6.html",title:"India\'s C-section surge is a payment-design problem",cat:"Policy",kw:"csection caesarean c-section nfhs nfhs-6 surgical birth delivery private hospital public facility maternal who ceiling reimbursement parity robson telangana bihar obstetric 27.2 54.1 payment design incentive",sum:"NFHS-6 puts India's C-section rate at 27.2%, up from 21.5%. The split underneath is the story: 54.1% in private facilities against a far lower public rate. WHO's 2015 position is that at population level rates above about 10% are not associated with lower maternal or newborn mortality, and WHO no longer recommends a target rate. The useful comparison is the private-versus-public gap, which points to payment design rather than clinical need."},
    {ed:65,url:"/edition-65-fertility-nfhs6.html",title:"India already stopped growing",cat:"Policy",kw:"fertility tfr total fertility rate nfhs nfhs-6 replacement 2.0 1.6 urban rural population momentum ageing demographic dividend states bihar kerala workforce pension",sum:"NFHS-6 puts India's total fertility rate at 2.0, below the replacement level of 2.1, and at 1.6 in urban areas. The population still rises on momentum from earlier cohorts, but the engine has stopped. States diverge sharply, and the ageing curve arrives before the income to pay for it, which reframes the demographic dividend argument."},
    {ed:66,url:"/edition-66-hollow-pyramid.html",title:"The hollow pyramid of rural specialists",cat:"Strategy",kw:"rural chc community health centre specialist vacancy sanctioned filled workforce health dynamics of india mohfw phc anaesthetist obstetrician paediatrician surgeon referral shortfall nursing",sum:"Two of every three specialist posts at India's rural Community Health Centres sit empty, and the shortfall worsens the higher you climb the referral chain. From MoHFW Health Dynamics of India 2023-24: surgeons, obstetricians, physicians and paediatricians are all far below sanctioned strength. India built the buildings and did not fill them, which is why referral pathways break at exactly the point they are needed."},
    {ed:67,url:"/edition-67-autism-measurement.html",title:"The number India doesn\'t have",cat:"Policy",kw:"autism asd prevalence inclen measurement diagnosis udid niramaya rci special educator screening rbsk deic neurodevelopmental disability registry census",sum:"India measures autism at about 1% from a single 2018 INCLEN study, runs no routine national count, and diagnoses at four to five years. Prevalence is known, entitlements are published, and everything in between is unmeasured: how many screened children reach diagnosis, and how many diagnosed adults are employed. The measurement stops exactly where accountability would start."},
    {ed:68,url:"/edition-68-one-fix-four-indias.html",title:"Four Indias, one shared bill",cat:"Strategy",kw:"four indias tiers out of pocket oop medicines jan aushadhi generic branded prescription molecule nha nfhs insurance cover value chain segmentation missing middle",sum:"NFHS-6 shows 60.2% household health cover, but that single number hides four different failures across four tiers of Indian households. Medicines are 60.3% of outpatient out-of-pocket spending for every tier, so the exposure is shared while the fix is not. Jan Aushadhi reaches 776 of 784 districts at 50 to 80% below branded prices and holds roughly 1% of the market. The shops exist; the prescriptions do not."},
    {ed:69,url:"/edition-69-gig-healthcare.html",title:"Can gig work fix India\'s health workforce?",cat:"Governance",kw:"gig work platform workforce doctors nurses niti aayog code on social security karnataka platform workers act aggregator hpr hfr abha credentialing accountability distribution financing",sum:"Gig platforms solve scheduling. India's health workforce crisis is distribution and financing, so gig work is a capacity instrument rather than an equity one. Health worker density is 26.5 registered but 11.0 active and adequately qualified per 10,000 against a WHO benchmark of 44.5. Accountability dissolves across three parties, and India publishes no healthcare-specific gig workforce data at all."},
    {ed:70,url:"/edition-70-swasth-bharat.html",title:"You cannot fix an org chart with an API",cat:"Governance",kw:"swasth bharat portal aggregator convergence api federated abdm abha u-win ni-kshay ncd janani pmndp scd sickle cell single login org chart budget targets reporting line maturity efficiency projection mohfw pib",sum:"The Swasth Bharat Portal puts multiple national health programmes behind one login on an API-based federated architecture. The diagnosis is right: India never had a digitisation problem, it had an org-chart problem. But each programme keeps its own budget, targets and reporting line, so convergence is real at the interface and absent underneath. The likely failure mode is a stall at stage two of five, a working portal that never reaches clinical convergence."},
    {ed:71,url:"/edition-71-mother-baby-journey.html",title:"India reaches almost every pregnant woman once. Then it loses her.",cat:"Policy",kw:"antenatal anc maternal newborn nfhs nfhs-6 iron folic acid ifa institutional delivery skilled attendance breastfeeding postnatal suman janani ssbsk high risk pregnancy completion arrival continuum inap",sum:"NFHS-6: 95.9% of pregnant women see a health worker once, 65.2% complete four check-ups, 37.8% finish 180 days of iron and folic acid, and 15.3% of toddlers eat an adequate diet. These are separate indicators rather than one cohort, but the pattern is consistent. India solved arrival and still measures arrival. Three of nine steps publish no number at all: before pregnancy, high-risk tracking, and care for sick newborns."},
    {ed:72,url:"/edition-72-india-financing-180.html",title:"India doesn\'t have a financing model. Nor do 94 other countries.",cat:"Strategy",kw:"health financing sha 2011 beveridge bismarck pooling purchasing revenue raising provider payment social health insurance tax funded mixed segmented atlas 180 countries who ghed esic pm-jay out of pocket",sum:"An atlas of 180 countries classified with the SHA 2011 financing schemes finds 95 of them, 53%, are mixed or segmented. The three textbook models cover only 79 between them. Europe is 3% mixed while Africa is 81%, so Beveridge and Bismarck describe the one region where the categories hold before being exported as a universal template. India runs five pooling arrangements side by side, and out-of-pocket spending is a mechanism rather than a model."},
    {ed:76,url:"/edition-76-nhcx-mandate.html",title:"Nobody will mandate NHCX. Your regulator, your insurer and your data law will.",cat:"Digital Health",kw:"nhcx dashboard statistics 867 hospitals 46,996 api requests pmjay 36,229 empanelled 16,746 private hospitals 2% 5% payer type national health claims exchange mandate mandatory irdai dpdp data protection may 2027 nmc medical colleges abdm hmis abha ghost patients insurer tpa cashless one hour three hour pre-authorisation discharge claims settlement 30 45 days ahpi bajaj allianz care health dhis incentive esushrut clinic staircase regulator shall endeavour parliamentary committee penalties",sum:"NHA dashboard, 22 Sep 2026: 46,996 government-scheme hospitals and 8.89 crore claim messages on NHCX; private insurance 867 hospitals and 30,246. Everyone is waiting for a notification that fills the private lane. It is not coming. Three doors are already opening: the DPDP deadline in May 2027, a medical education regulator that has started punishing colleges, and an insurer that has to answer a one-hour clock. Traction arrives as a staircase, largest and most regulated first."},
    {ed:75,url:"/edition-75-covered-not-paid.html",title:"Covered, not paid",cat:"Policy",kw:"health insurance irdai claims settlement repudiation disallowance incurred claims ratio icr cashless reimbursement payer mix premium per life government sponsored group individual nfhs-6 coverage caesarean institutional birth private hospital public facility receivable days denial management star health niva bupa care health oriental",sum:"IRDAI FY25: insurers settled 87.5 per cent of health claims by count but only 71.1 per cent by value. Of Rs 1,32,488 crore claimed, Rs 29,933 crore never arrived, and Rs 18,521 crore of that was disallowance rather than rejection, money cut from bills insurers had agreed to settle, carrying a claim count of zero. Incurred claims ratios span 41 points, from 61 per cent at standalone health insurers to 103 per cent at public sector ones, with no overlap. Government schemes cover 42 per cent of lives for 8 per cent of premium. Coverage rose 19 points while the caesarean rate reached 54.1 per cent in private facilities against 16.9 per cent in public, with the private rate higher in all 21 states with data."},
    {ed:74,url:"/edition-74-healthcare-cost-effectiveness.html",title:"A cheaper treatment can still waste money",cat:"Strategy",kw:"cost effectiveness hta health technology assessment htain icer qaly daly threshold budget impact affordability cost saving cost minimisation cost benefit cost utility cost consequence roi return on investment efficiency dea comparator outcome time horizon nice value based procurement pricing reimbursement decision willingness to pay",sum:"Price tells you what one unit costs, not what it achieves, for whom, or over what period. Healthcare has nine ways of asking whether something is worth it and they give different answers to the same question: cost-effectiveness, cost-utility, cost-benefit, cost-minimisation, budget impact, ROI, cost of illness, cost-consequence and efficiency. A worked example where the price gap is not the decision, four phrases people use interchangeably and should not, and why India already has the machinery in HTAIn under the Department of Health Research."},
    {ed:73,url:"/edition-73-hospital-capital.html",title:"Adding beds is the easy part. Making each bed work is the hard part.",cat:"Strategy",kw:"hospital beds occupancy arpob ebitda margin bed maturity private equity kkr medicover max healthcare manipal fortis apollo kims icra crisil length of stay alos throughput capex expansion productivity",sum:"Indian hospital groups grew revenue 21% and added 15% more working beds last quarter while margins stayed near 23%. ICRA expects 14,500 more beds at about Rs 30,000 crore across FY26 and FY27. A new bed loses money before it makes any, moving through six stages from built to mature. The claim that filling beds widens margins does not survive testing: the relationship points one way in FY25 and the other in FY26. Shorter stays and price per bed-day show clearer effects. KKR's $1.4bn Medicover deal is largely a debt story, with 19 of 25 hospitals already profitable."}
  ];
  ...
  function score(qTokens, ed){
    var hay = (ed.kw + " " + ed.title + " " + ed.cat).toLowerCase(), s = 0;
    qTokens.forEach(function(t){
      if(hay.indexOf(t) > -1) s += 2;
      else { var stem = t.length > 5 ? t.slice(0,5) : t; if(hay.indexOf(stem) > -1) s += 1; }
    });
    return s;
  }
  ...
  var CSS = '\
.hpa-bubble{position:fixed;right:20px;bottom:20px;z-index:9998;display:inline-flex;align-items:center;gap:8px;\
  background:linear-gradient(135deg,#0FA3A3,#2E7CB8);color:#fff;border:0;border-radius:999px;cursor:pointer;\
  padding:13px 18px;font-family:Manrope,system-ui,sans-serif;font-size:14.5px;font-weight:700;\
  box-shadow:0 10px 26px -8px rgba(15,163,163,.65);transition:transform .18s ease,box-shadow .18s ease}\
.hpa-bubble:hover{transform:translateY(-2px);box-shadow:0 14px 30px -8px rgba(15,163,163,.75)}\
.hpa-bubble.hide{display:none}\
.hpa-panel{position:fixed;right:20px;bottom:20px;z-index:9999;width:380px;max-width:calc(100vw - 32px);\
  height:520px;max-height:calc(100vh - 40px);background:#fff;border:1px solid #E2E8EC;border-radius:16px;\
  box-shadow:0 24px 60px -20px rgba(27,58,75,.5);display:none;flex-direction:column;overflow:hidden;\
  font-family:Manrope,system-ui,sans-serif}\
.hpa-panel.open{display:flex}\
html[data-theme="dark"] .hpa-panel{background:#16252C;border-color:#2C3F47}\
.hpa-head{background:#1B3A4B;color:#fff;padding:13px 15px;display:flex;justify-content:space-between;align-items:center;flex:none}\
.hpa-title{font-size:14.5px;font-weight:800;letter-spacing:-.01em}\
.hpa-sub{font-size:11.5px;opacity:.82;margin-top:2px}\
.hpa-x{background:none;border:0;color:#fff;cursor:pointer;padding:4px;line-height:0;opacity:.85}\
.hpa-x:hover{opacity:1}\
.hpa-body{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px}\
.hpa-msg{max-width:88%;font-size:13.5px;line-height:1.6;padding:10px 13px;border-radius:13px}\
.hpa-q{align-self:flex-end;background:#0FA3A3;color:#fff;border-bottom-right-radius:3px}\
.hpa-a{align-self:flex-start;background:#F1F6F7;color:#22333B;border-bottom-left-radius:3px}\
html[data-theme="dark"] .hpa-a{background:#1E313A;color:#DCE9EC}\
.hpa-a b{color:#1B3A4B}\
html[data-theme="dark"] .hpa-a b{color:#E8F4F4}\
.hpa-cites{margin-top:9px;padding-top:9px;border-top:1px solid rgba(90,107,114,.22);display:flex;flex-direction:column;gap:5px}\
.hpa-cite{font-size:12.5px;color:#0FA3A3;text-decoration:none;font-weight:700}\
.hpa-cite:hover{text-decoration:underline}\
.hpa-chips{display:flex;flex-wrap:wrap;gap:6px;margin-top:2px}\
.hpa-chip{font-size:12px;font-weight:600;color:#5A6B72;background:transparent;border:1px solid #E2E8EC;\
  border-radius:999px;padding:6px 11px;cursor:pointer}\
.hpa-chip:hover{border-color:#0FA3A3;color:#0FA3A3}\
html[data-theme="dark"] .hpa-chip{border-color:#2C3F47;color:#A8BDC4}\
.hpa-foot{flex:none;border-top:1px solid #E2E8EC;padding:10px 12px;display:flex;gap:8px;align-items:center}\
html[data-theme="dark"] .hpa-foot{border-top-color:#2C3F47}\
.hpa-in{flex:1;border:1px solid #E2E8EC;border-radius:999px;padding:10px 14px;font-family:inherit;font-size:13.5px;\
  background:#fff;color:#22333B;outline:none}\
.hpa-in:focus{border-color:#0FA3A3}\
html[data-theme="dark"] .hpa-in{background:#0F1B22;border-color:#2C3F47;color:#DCE9EC}\
.hpa-send{flex:none;width:36px;height:36px;border-radius:50%;border:0;cursor:pointer;\
  background:linear-gradient(135deg,#0FA3A3,#2E7CB8);color:#fff;display:inline-flex;align-items:center;justify-content:center}\
.hpa-more{display:block;text-align:center;font-size:12px;color:#5A6B72;padding:6px 0 2px;text-decoration:none}\
.hpa-more:hover{color:#0FA3A3}\
@media(max-width:560px){\
 .hpa-panel{right:0;bottom:0;width:100vw;max-width:100vw;height:88vh;border-radius:16px 16px 0 0}\
 .hpa-bubble{right:14px;bottom:14px;padding:12px 16px}\
}';
  ...
  wrap.innerHTML = '\
<button class="hpa-bubble" id="hpaBubble" type="button" aria-label="Ask Healthcare Pulse">\
  <svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-3.6-.7L3 21l1.9-5A8.4 8.4 0 0 1 12 3.1a8.4 8.4 0 0 1 9 8.4z"/></svg>Ask</button>\
<div class="hpa-panel" id="hpaPanel" role="dialog" aria-label="Ask Healthcare Pulse">\
  <div class="hpa-head">\
    <div><div class="hpa-title">Ask Healthcare Pulse</div><div class="hpa-sub">Answers grounded in the archive</div></div>\
    <button class="hpa-x" id="hpaClose" type="button" aria-label="Close">\
      <svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>\
  </div>\
  <div class="hpa-body" id="hpaBody"></div>\
  <div class="hpa-foot">\
    <input class="hpa-in" id="hpaIn" type="text" placeholder="Ask about Indian healthcare…" autocomplete="off">\
    <button class="hpa-send" id="hpaSend" type="button" aria-label="Send">\
      <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>\
  </div>\
</div>';
  document.body.appendChild(wrap);

  var bubble=document.getElementById('hpaBubble'), panel=document.getElementById('hpaPanel'),
      body=document.getElementById('hpaBody'), input=document.getElementById('hpaIn'),
      send=document.getElementById('hpaSend'), close=document.getElementById('hpaClose');

  function add(cls, html){
    var d=document.createElement('div'); d.className='hpa-msg '+cls; d.innerHTML=html;
    body.appendChild(d); body.scrollTop=body.scrollHeight; return d;
  }
  function greet(){
    add('hpa-a','Ask me anything about Indian healthcare. I answer from what I have actually published, and I will point you to the edition.'
      +'<div class="hpa-chips">'
      +'<button class="hpa-chip" data-q="What is India\'s C-section rate?">C-section rates</button>'
      +'<button class="hpa-chip" data-q="How does ABHA work?">ABHA</button>'
      +'<button class="hpa-chip" data-q="What does DPDPA change for hospitals?">DPDPA</button>'
      +'<button class="hpa-chip" data-q="Why are rural specialist posts vacant?">Rural specialists</button>'
      +'</div>');
  }
  function cites(top){
    return '<div class="hpa-cites">' + top.map(function(x){
      return '<a class="hpa-cite" href="'+x.ed.url+'">Edition '+x.ed.ed+' · '+esc(x.ed.title)+' &rarr;</a>';
    }).join('') + '</div>';
  }
  function notCovered(q){
    add('hpa-a','I have not written about that yet. '
      +'<a class="hpa-cite" href="mailto:mayank@mayankmadhur.in?subject='+encodeURIComponent('Topic suggestion: '+q)+'">Suggest it as a topic &rarr;</a>');
  }

  function ask(){
    var q=(input.value||'').trim(); if(!q) return;
    add('hpa-q', esc(q)); input.value='';
    var qt=tokenize(q);
    if(!qt.length){ notCovered(q); return; }
    var ranked=EDITIONS.map(function(ed){return {ed:ed,s:score(qt,ed)};})
      .filter(function(x){return x.s>0;}).sort(function(a,b){return b.s-a.s;});
    if(!ranked.length || ranked[0].s<2){ notCovered(q); return; }
    var top=ranked.slice(0,3);
    var thinking=add('hpa-a','Thinking through what I have written…');
    ...
    .then(function(d){
      var a=(d&&d.answer)?d.answer:""; if(!a) throw 0;
      thinking.innerHTML=esc(a).replace(/\n/g,'<br>')+cites(top)
        +'<a class="hpa-more" href="/ask/">Open the full Ask page &rarr;</a>';
      body.scrollTop=body.scrollHeight;
    })
    .catch(function(){
      thinking.innerHTML='Here is what I have written that covers it:'+cites(top);
      body.scrollTop=body.scrollHeight;
    });
  }

  function open(){ panel.classList.add('open'); bubble.classList.add('hide');
    if(!body.children.length) greet(); setTimeout(function(){input.focus();},80); }
  function shut(){ panel.classList.remove('open'); bubble.classList.remove('hide'); }
  ...
  body.addEventListener('click', function(e){
    var c=e.target.closest('.hpa-chip'); if(!c) return;
    input.value=c.getAttribute('data-q'); ask();
  });
